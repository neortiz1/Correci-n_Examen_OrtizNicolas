...
if (isset($_GET['nuevaFuncion'])) {
    $consulta = mysqli_query($enlace, "SELECT COD_FUNCIONALIDAD AS id FROM seg_funcionalidad ORDER BY seg_funcionalidad.COD_FUNCIONALIDAD DESC LIMIT 1");
    while ($row = mysqli_fetch_assoc($consulta)) { 
        $id = $row['id']; 
    }
    $id = $id+1;
    $consulta = mysqli_query($enlace, "INSERT INTO seg_funcionalidad(COD_FUNCIONALIDAD,COD_MODULO,URL_PRINCIPAL,NOMBRE,DESCRIPCION) VALUES ('".$id."','".$_POST['modulo']."','".$_POST['urlP']."','".$_POST['nombre']."','".$_POST['descripcion']."')");  
    if (mysqli_affected_rows($enlace)){ 
        $respuesta = "exito";   
    }
    if($respuesta=="exito"){
